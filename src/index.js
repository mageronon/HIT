...
var listCategoryApi = new Vue({
  ...
  methods: {
    open(goods){
      ...
      $.ajax({
        url: "https://nit.tron.net.ua/api/product/" + goods.id,
        type: "GET",
        cache: false,
    ...
    $.ajax({
      ...
      success: function ( data ) {
        ...
        for (var item in data) {
          ...
          $.ajax({
            url: "https://nit.tron.net.ua/api/product/list/category/" + data[item].id,
            type: "GET",
            cache: false,
            success: function ( data1 ) {
              var id = this.url.split('/')[7].split('?')[0];
              listCategoryApi.goods_by_category.push({"id" : id, "data": data1});
            }
          });
        }
      }
...
var myModal = new Vue({
  ...
  methods: {
    ...
    clearAll() {
      ...
      var order = {};
      for (var index in buy) {
        order[buy[index].id] = buy[index].count
      }
      // add order
      $.post('https://nit.tron.net.ua/api/order/add',
        {
          token: "4YDm6Y_hkvmitcIeQ_jm",
